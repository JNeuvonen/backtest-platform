...
from quantstats_lumi import __version__
from quantstats_lumi import plots as _plots
from quantstats_lumi import stats as _stats
from quantstats_lumi import utils as _utils
...
def html(
    ...
    **kwargs,
    ...
    cagr_periods_per_year = kwargs.get("cagr_periods_per_year", 365)
    mtrx = metrics(
        returns=returns,
        benchmark=benchmark,
        rf=rf,
        ...
        cagr_periods_per_year=cagr_periods_per_year,
    )[2:]
...
def metrics(
    ...
    **kwargs,
    ...
    metrics["CAGR% (Annual Return) "] = (
        _stats.cagr(df, rf, compounded, cagr_periods_per_year) * pct
    )
    ...
    metrics["3Y (ann.) %"] = (
        _stats.cagr(df[df.index >= d], 0.0, compounded, cagr_periods_per_year) * pct
    )
    ...
    metrics["All-time (ann.) %"] = (
        _stats.cagr(df, 0.0, compounded, cagr_periods_per_year) * pct
    )
